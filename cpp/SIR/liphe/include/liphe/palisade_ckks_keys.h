#ifndef __PALISADE_CKKS_KEYS__
#define __PALISADE_CKKS_KEYS__

#include <memory>

#include <palisade.h>

class PalisadeCkksKeys {
public:
	struct Params {
		uint32_t multDepth;
...
public:
	PalisadeCkksKeys() {}
	PalisadeCkksKeys(const PalisadeCkksKeys &h) : _keyPair(h._keyPair), _context(h._context), _nSlots(h._nSlots), _sec(h._sec) {}
	...
	void initKeys(int multDepth, int scaleFactorBits, int slots, const std::vector<int> &rotations, const lbcrypto::SecurityLevel &sec = lbcrypto::HEStd_128_classic) {
		...
		lbcrypto::RescalingTechnique rsTech = lbcrypto::APPROXRESCALE;
